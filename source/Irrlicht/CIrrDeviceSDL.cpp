...
//		SDL_Flags |= SDL_WINDOW_DOUBLEBUFFER;
	// create window
...
bool CIrrDeviceSDL::createWindow()
{
	...
	if (CreationParams.DriverType == video::EDT_OPENGL)
	{
            ...
            SDL_GL_SetAttribute ( SDL_GL_DOUBLEBUFFER , 1 ) ;
            SDL_GL_SetAttribute ( SDL_GL_DEPTH_SIZE , 24 ) ;
            if(( Screen = SDL_CreateWindow ( "Space " , 100 , 100 , screenx , screeny, FLAGS ) ) == NULL ) 
            {
        ...
        if ( !Screen ) 
	{
        ...
        if( !Context )
        {
            os::Printer::log( "Could not initialize OpenGL Context!" );
            return false;
        }
